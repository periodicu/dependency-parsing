...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        # build raw input representation as words
        rawirep = []

        # print statements for debugging, beware ye who enter here
        # print(words, "WORDS")
        # print(pos, "POS")
        # print(state.stack, "SYACK")
        # print(state.buffer, "BUFFER")

        for i in range(-1, -4, -1):
            if i >= -len(state.stack):
                rawirep.append(state.stack[i])
            else:
                rawirep.append(None)
        
        for i in range(-1, -4, -1):
            if i >= -len(state.buffer):
                rawirep.append(state.buffer[i])
            else:
                rawirep.append(None)
        ...
        for i in rawirep:
            if i is None:
                ...
                enco.append(self.word_vocab.get(words[i].lower(), 2))
        
        return np.array(enco)


    def get_output_representation(self, output_pair):  

        returnMe = self.output_labels[output_pair]
        return keras.utils.to_categorical(returnMe, num_classes=91)
